              ...
              >
                <div>
                  <div className="flex items-center justify-between">
                    <div className="p-2 rounded-md bg-surface-subtle border border-border text-status-info">
                      <Icon className="w-4 h-4" />
                    </div>
                    <StatusBadge status={preset.request.event_type} type="event" />
                  </div>
                  <h3 className="text-sm font-semibold text-fg mt-3">{preset.name}</h3>
                  <p className="text-xs text-fg-muted mt-1">{preset.description}</p>
                  <div className="mt-3 font-mono text-xs font-bold text-fg">
        ...
        <div className="bg-surface p-6 rounded-lg border border-border space-y-4">
          ...
          >
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-fg-muted block mb-1">Event Type</label>
                <select
              ...
              <div>
                <label className="text-fg-muted block mb-1">Payment Method</label>
                <select
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-fg-muted block mb-1">Amount (INR)</label>
                <input
                  type="number"
                  step="0.01"
                  value={form.amount}
              ...
              <div>
                <label className="text-fg-muted block mb-1">Customer Name</label>
                <input
                  type="text"
            ...
            <div>
              <label className="text-fg-muted block mb-1">Customer Email</label>
              <input
                type="email"
            ...
            {form.event_type === "payment.failed" && (
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-fg-muted block mb-1">Error Code</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-fg-muted block mb-1">Error Description</label>
                  <input
                    type="text"
        ...
        <div className="bg-surface rounded-lg border border-border flex flex-col overflow-hidden">
          <div className="flex items-center justify-between px-5 py-3.5 border-b border-border bg-surface-subtle">
            <div className="flex items-center space-x-2">
              ...
              <h2 className="text-xs font-semibold text-fg tracking-wide font-mono uppercase">
                PIPELINE EXECUTION CONSOLE
              </h2>
            </div>
            <span className="text-[10px] font-mono text-fg-faint">Live Stream</span>
          </div>
            ...
            {logs.map((log, i) => (
              <div
                ...
                className={`p-2 rounded border leading-relaxed ${
                  log.type === "success"
                    ? "bg-status-success-bg/40 border-status-success-border/40 text-status-success"
                    : log.type === "error"
                    ? "bg-status-danger-bg/40 border-status-danger-border/40 text-status-danger"
                    : "bg-surface border-border/80 text-fg-secondary"
                }`}
              >
                <span className="text-[10px] text-fg-faint mr-2">[{log.timestamp}]</span>
                {log.text}
              </div>
            ))}
          </div>
          ...
          {lastResult && (
            <div className="p-3 border-t border-border bg-surface text-xs font-mono flex items-center justify-between text-fg-secondary">
              <span>Last Event ID: <strong className="text-fg">{lastResult.razorpay_event_id}</strong></span>
              {lastResult.case_number && (
                <span className="text-status-info font-semibold">Case: {lastResult.case_number}</span>
              )}
            </div>
          )}
        </div>
