    ...
    <AppShell onRefresh={() => mutate()} isRefreshing={isValidating}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-border pb-6">
        ...
        <div className="text-xs font-mono text-slate-400 bg-surface px-3 py-1.5 rounded-lg border border-border">
          Total Customers: <span className="text-white font-bold">{data?.total || 0}</span>
        </div>
      </div>
          ...
          <div className="space-y-6 text-xs">
            ...
            <div className="p-4 rounded-lg bg-surface-subtle border border-border space-y-3">
              ...
              <div className="grid grid-cols-2 gap-4 pt-2">
                ...
                <div>
                  <span className="text-slate-400 font-mono">Total Transactions</span>
            ...
            <div className="space-y-3">
              ...
              <div className="p-4 rounded-lg bg-surface-subtle border border-border space-y-2 font-mono">
                <div className="flex justify-between">
                  ...
                  <span className="text-white">{selectedCustomer.name || "N/A"}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400">Email:</span>
                  <span className="text-blue-400">{selectedCustomer.email || "N/A"}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400">Phone:</span>
                  <span className="text-slate-300">{selectedCustomer.phone || "N/A"}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400">Razorpay Customer ID:</span>
                  <span className="text-slate-400">{selectedCustomer.razorpay_customer_id || "None"}</span>
                </div>
              </div>
            </div>
          </div>
