      ...
      <div className="flex items-center justify-between text-xs">
        <div className="flex items-center space-x-4 font-mono">
          <div className="flex items-center space-x-1.5">
            ...
            <span className="text-slate-300">Revenue at Risk (Failed)</span>
          </div>
          <div className="flex items-center space-x-1.5">
            <span className="w-2.5 h-2.5 rounded-sm bg-emerald-500"></span>
            <span className="text-slate-300">Revenue Secured (Captured)</span>
          </div>
        </div>
        <span className="text-slate-500 text-[11px] font-mono">Past 7 Days Activity</span>
      </div>
          ...
          return (
            <div key={index} className="flex-1 flex flex-col items-center h-full justify-end group relative">
              ...
              <div className="opacity-0 group-hover:opacity-100 transition-opacity absolute -top-12 z-20 pointer-events-none bg-surface-elevated border border-border p-2 rounded shadow-xl text-[10px] font-mono whitespace-nowrap">
                <div className="text-white font-semibold">{dateLabel}</div>
                <div className="text-rose-400">At Risk: {formatINR(failed)}</div>
                <div className="text-emerald-400">Secured: {formatINR(captured)}</div>
              </div>
              ...
              <div className="w-full max-w-[28px] flex items-end gap-1 h-full">
                ...
                <div
                  ...
                  className="w-1/2 bg-rose-500/80 hover:bg-rose-500 rounded-t transition-all duration-300"
                />
                {/* Captured Bar */}
                <div
                  style={{ height: `${Math.max(capturedHeight, 4)}%` }}
                  className="w-1/2 bg-emerald-500/80 hover:bg-emerald-500 rounded-t transition-all duration-300"
                />
              </div>
